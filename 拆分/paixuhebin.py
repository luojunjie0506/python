...
if __name__ == '__main__':
    ...
    list3 = []  # 存放每个表的行数
    list4 = []  # 存放每个表的列数
    sheet_list = ['门店信息表','业绩汇总表','补扣款备注','200元以下合计','店补发放','服务费清单原始数据']  # sheet名列表
    save_path='D:\\xx.xls' #合并后excel的存放路径
    ...
    for a in file_list:
        ...
        for btname in data:
            ...
            df.to_excel(writer, encoding='utf-8', sheet_name=a[7:-5], index=None)  # 写入sheet中 index无索引
            writer.save()
            writer.close()
            print(a[7:-5])
            break
    ...
    # 打开sheet表
    sheet0 = data.sheet_by_name('门店信息表')  # 门店信息表
    sheet1 = data.sheet_by_name('业绩汇总表')  # 业绩汇总表
    sheet2 = data.sheet_by_name('补扣款备注')  # 补扣款备注
    sheet3 = data.sheet_by_name('200元以下合计')  # 200元以下合计
    sheet4 = data.sheet_by_name('店补发放') # 店补发放
    sheet5 = data.sheet_by_name('服务费清单原始数据')  # 服务费清单原始数据


    # 获取每表的行数
    for i in range(0, 6):
        rows = data.sheet_by_name(sheet_list[i]).nrows
        list3.append(rows)

    # 获取每表的列数
    for i in range(0, 6):
        cols = data.sheet_by_name(sheet_list[i]).ncols
        list4.append(cols)
